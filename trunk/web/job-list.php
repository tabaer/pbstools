...
if ( isset($_POST['system']) )
  {
    $db =db_connect();
    $sql = "SELECT system, jobid, username, account, jobname, nproc, nodes, mem_req, mem_kb, FROM_UNIXTIME(submit_ts), FROM_UNIXTIME(start_ts), FROM_UNIXTIME(end_ts), walltime_req, walltime, ".charges($db,$_POST['system'],$_POST['start_date'],$_POST['end_date'])." AS charges, queue, IF(script IS NULL,'interactive','batch') AS type, sw_app AS software FROM Jobs WHERE system LIKE '".$_POST['system']."' AND ( ".dateselect("during",$_POST['start_date'],$_POST['end_date'])." ) ORDER BY start_ts;";
    #echo "<PRE>".$sql."</PRE>\n";
