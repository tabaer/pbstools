	...
	if ( $key!='system' && $key!='start_date' && $key!='end_date' )
	  {
	    ...
	    $sql = "SELECT ".xaxis_column("quarter").", COUNT(jobid) AS jobs, SUM(nproc*TIME_TO_SEC(walltime))/3600.0 AS cpuhours, SUM(TIME_TO_SEC(cput))/3600.0 AS cpuhours_alt, COUNT(DISTINCT(username)) AS users, COUNT(DISTINCT(groupname)) AS groups FROM Jobs WHERE system LIKE '".$_POST['system']."' AND ( ";
	    ...
	    echo "<TABLE border=1>\n";
	    echo "<TR><TH>quarter</TH><TH>jobs</TH><TH>cpuhours</TH><TH>cpuhours_alt</TH><TH>users</TH><TH>groups</TH></TR>\n";
