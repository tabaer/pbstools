...
if ( isset($_POST['jobid']) )
  { 
    $title="Job info for ".$_POST['system']." hostnames like ".$_POST['jobid'];
  } 
 else
  {
    $title="Job info";
...
if ( isset($_POST['jobid']) )
  {
    ...
    $sql = $sql." FROM keeneland_gpu_stats WHERE hostname LIKE '".$_POST['jobid']."%'";
    ...
    if ( isset($_POST['csv']) )
    {
        ...
        result_as_csv($csvresult,$columns,$_POST['system']."-job_info_by_hostname-".$_POST['start_date']."-".$_POST['end_date']);
    }
    if ( isset($_POST['xls']) )
    {
        $xlsresult = db_query($db,$sql);
        result_as_xls($xlsresult,$columns,$_POST['system']."-job_info_by_hostname-".$_POST['start_date']."-".$_POST['end_date']);
    }
    if ( isset($_POST['ods']) )
    {
        $odsresult = db_query($db,$sql);
        result_as_ods($odsresult,$columns,$_POST['system']."-job_info_by_hostname-".$_POST['start_date']."-".$_POST['end_date']);
    }
  ...
  {
    begin_form("gpu-info-by-hostname.php");

    text_field("Hostname like","jobid",8);
    system_chooser();
