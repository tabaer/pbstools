...
int mpi_fileattr_define() {    
    ...
    lena[0] = MAX_PATH;    /* file1.pathname has length of MAX_PATH chars*/
    MPI_Address(&file1.pathname,&loca[0]); 
    ...
    MPI_Address(&file1.atime,&loca[3]); 
    ...
    MPI_Address(&file1.mtime,&loca[4]); 
    ...
    MPI_Address(&file1.ctime,&loca[5]); 
    loca[5] -= baseaddress;  /* byte address relative to start of structure */
    typa[5] = MPI_UNSIGNED_LONG;    
    
    MPI_Type_struct(6, lena, loca, typa, &MPI_FileAttr);
    MPI_Type_commit(&MPI_FileAttr);
