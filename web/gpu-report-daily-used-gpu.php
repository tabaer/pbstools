...
if ( isset($_POST['system']) )
  { 
    $title="Report for ".$_POST['system']." on Daily GPU Utilization";
  } 
 else
  {
    $title="Report Form";
...
if ( isset($_POST['system']) )
  {
    ...
    for($i = $start; $i <= $end; $i = mktime(0, 0, 0, date("m", $i), date("d", $i)+1, date("Y", $i)))
    {
		...
		$d = date("Y-m-d",$i);
        $sql = $sql."SELECT '$d' AS Date, hostname, gpu_number FROM keeneland_gpu_stats WHERE gpu_utilization > 0 AND time_epoch BETWEEN $i AND $i_next";
        if ( $i_next+1 <= $end )
        {
            $sql = $sql." UNION ";
        }
    ...
    $sql = "SELECT Date, count(gpu_number) FROM ( $sql ) subQuery GROUP BY Date";

    $result =& db_query($db, $sql);
    $columns = array("Date", "Number of GPUs Used");
    if ( isset($_POST['csv']) )
    {
        $csvresult = db_query($db,$sql);
        result_as_csv($csvresult,$columns,$_POST['system']."-daily-used-gpu-".$_POST['start_date']."-".$_POST['end_date']);
    }
    if ( isset($_POST['xls']) )
    {
        $xlsresult = db_query($db,$sql);
        result_as_xls($xlsresult,$columns,$_POST['system']."-daily-used-gpu-".$_POST['start_date']."-".$_POST['end_date']);
    }
    if ( isset($_POST['ods']) )
    {
        $odsresult = db_query($db,$sql);
        result_as_ods($odsresult,$columns,$_POST['system']."-daily-used-gpu-".$_POST['start_date']."-".$_POST['end_date']);
    }
    db_disconnect($db);
    //bookmarkable_url();
  }
  ...
  {
    begin_form("gpu-report-daily-used-gpu.php");
