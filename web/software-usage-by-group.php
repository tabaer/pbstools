...
if ( isset($_POST['system']) )
  {
	...
	if ( $key!='system' && $key!='start_date' && $key!='end_date' &&
	     $key!='datelogic' )
	  {
	    ...
	    $sql .= "sw_app='".$key."'";
	    $sql .= " ) AND ( ".dateselect($_POST['datelogic'],$_POST['start_date'],$_POST['end_date'])." ) GROUP BY groupname UNION SELECT 'TOTAL:',COUNT(jobid) AS jobs, SUM(".cpuhours($db,$_POST['system'],$_POST['start_date'],$_POST['end_date']).") AS cpuhours, SUM(".charges($db,$_POST['system'],$_POST['start_date'],$_POST['end_date']).") AS charges FROM Jobs WHERE ( ".sysselect($_POST['system'])." AND groupname IS NOT NULL AND ( ";
	    $sql .= "sw_app='".$key."'";
