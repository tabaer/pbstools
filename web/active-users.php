...
if ( isset($_POST['system']) )
  {
    $db = db_connect();
    $sql = "SELECT username, groupname, account, COUNT(jobid) AS jobs, SUM(".cpuhours($db,$_POST['system'],$_POST['start_date'],$_POST['end_date'],$_POST['datelogic']).") AS cpuhours, SUM(".charges($db,$_POST['system'],$_POST['start_date'],$_POST['end_date'],$_POST['datelogic']).") AS charges FROM Jobs WHERE ( ".sysselect($_POST['system'])." ) AND ( ".dateselect($_POST['datelogic'],$_POST['start_date'],$_POST['end_date'])." ) GROUP BY username, groupname, account ORDER BY ".$_POST['order']." DESC LIMIT ".$_POST['limit'];
    #echo "<PRE>".$sql."</PRE>\n";
    ...
    echo "<TABLE border=\"1\">\n";
    echo "<TR><TH>user</TH><TH>group</TH><TH>account</TH><TH>job count</TH><TH>CPU-hours</TH><TH>charges</TH></TR>\n";
